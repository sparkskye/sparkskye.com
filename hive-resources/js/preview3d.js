...
export class CardPreview {
  ...
  async init(modelUrl) {
    ...
    this.renderer = makeRenderer(this.canvas, { preserveDrawingBuffer: true });
  ...
  freezeToImage() {
    if (!this.canvas || this.disposed) return null;

    try {
      const dataUrl = this.canvas.toDataURL("image/png");
      const img = document.createElement("img");
      img.className = "card__thumb card__thumb--rendered";
      img.alt = "";
      img.src = dataUrl;
