import { fetchMaps, fetchMapGames, fileViewUrl, trackedDownloadUrl, fetchDownloadCount } from "./api.js";
import {
  qs,
  debounce,
  setUrlParam,
...
const els = {
  ...
  modalDownloadsGlb: qs("#modalDownloadsGlb"),
  modalDownloadsPng: qs("#modalDownloadsPng"),
};
...
function clampMapView() {
  ...
  const scaledW = iw * mapView.scale;
  const scaledH = ih * mapView.scale;

  // Allow panning past the image edges a bit (so you can "fly off" the map),
  // and rely on HOME to snap back to a clean centered fit.
  const overscroll = Math.min(420, Math.max(160, Math.min(vw, vh) * 0.35));
...
function applyMapTransform() {
  ...
  clampMapView();
  mapView.img.style.transform = `translate(-50%, -50%) translate(${mapView.x}px, ${mapView.y}px) scale(${mapView.scale})`;
  if (mapView.helper) {
    mapView.helper.textContent = `${Math.round(mapView.scale * 100)}%`;
  }
}

function applyFitScale_(fitScale) {
  // Allow zooming out past the fitted view, so you can see "off-map" space.
  // When the map is smaller than the viewport, also allow zooming down well below 100%.
  const minScale = Math.max(0.02, Math.min(fitScale, 1) * 0.25);
  mapView.minScale = minScale;
  mapView.scale = fitScale;
  mapView.x = 0;
  mapView.y = 0;
  applyMapTransform();
}
...
function scheduleFitMapToView(tries = 0) {
  ...
  // Layout is ready (or close enough)
  fitMapToView();
}

function zoomMap(multiplier) {
  if (!mapView.img) return;
  const next = Math.min(mapView.maxScale, Math.max(mapView.minScale, mapView.scale * multiplier));
  mapView.scale = next;
  applyMapTransform();
}
...
function mountMapPreview(it, thumbSrc) {
  ...
  mapView.pointerMoveHandler = (ev) => {
    ...
    if (mapView.pinching && mapView.pointers.size >= 2) {
      ...
      const nextScale = mapView.pinchStartScale * (dist / (mapView.pinchStartDist || 1));
      mapView.scale = Math.min(mapView.maxScale, Math.max(mapView.minScale, nextScale));
      applyMapTransform();
      return;
    }
...
function setMapDownloadStat(el, text, isOff = false) {
  if (!el) return;
  el.textContent = text;
  el.classList.toggle("is-off", !!isOff);
}

async function refreshMapDownloadStats(item) {
  const reqId = ++state.downloadCountRequest;
  setMapDownloadStat(els.modalDownloadsGlb, "glb downloads: ...", true);
  setMapDownloadStat(els.modalDownloadsPng, "png downloads: ...", true);

  const glbPromise = item?.glbId ? fetchDownloadCount(item.glbId, "map-glb") : Promise.resolve({ enabled: false, count: null });
  const pngPromise = item?.thumbId ? fetchDownloadCount(item.thumbId, "map-png") : Promise.resolve({ enabled: false, count: null });

  try {
    const [glbStats, pngStats] = await Promise.all([glbPromise, pngPromise]);
    if (reqId !== state.downloadCountRequest) return;

    if (glbStats?.enabled) setMapDownloadStat(els.modalDownloadsGlb, `glb downloads: ${Number(glbStats.count) || 0}`, false);
    else setMapDownloadStat(els.modalDownloadsGlb, item?.glbId ? "glb tracking off" : "no glb file", !item?.glbId);

    if (pngStats?.enabled) setMapDownloadStat(els.modalDownloadsPng, `png downloads: ${Number(pngStats.count) || 0}`, false);
    else setMapDownloadStat(els.modalDownloadsPng, item?.thumbId ? "png tracking off" : "no png file", !item?.thumbId);
  } catch {
    if (reqId !== state.downloadCountRequest) return;
    setMapDownloadStat(els.modalDownloadsGlb, "glb unavailable", true);
    setMapDownloadStat(els.modalDownloadsPng, "png unavailable", true);
  }
}

function bumpMapDownloadStat(el) {
  if (!el) return;
  const match = el.textContent.match(/(\d+)/);
  if (!match) return;
  const next = Number(match[1]) + 1;
  const prefix = el.id === "modalDownloadsPng" ? "png downloads" : "glb downloads";
  setMapDownloadStat(el, `${prefix}: ${next}`, false);
}
...
function openModal(it, opts = {}) {
  ...
  els.modalDownload.onclick = async (ev) => {
    ...
    if (state.activePreviewId === state.previewId) bumpMapDownloadStat(els.modalDownloadsGlb);
  };
  ...
  els.modalDownloadPng.onclick = async (ev) => {
    ...
    if (pngDownload) {
      ...
      if (state.activePreviewId === state.previewId) bumpMapDownloadStat(els.modalDownloadsPng);
      return;
    }
    if (it.thumbUrl) {
