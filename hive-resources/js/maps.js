...
function fitMapToView() {
  ...
  mapView.minScale = Math.min(vw / iw, vh / ih);
  if (!Number.isFinite(mapView.minScale) || mapView.minScale <= 0) mapView.minScale = 1;
...
function mountMapPreview(it, thumbSrc) {
  ...
  zoomGroup.appendChild(createMapControl("HOME", "btn btn--nav", () => fitMapToView()));

  const panGroup = document.createElement("div");
  panGroup.className = "map-preview__group map-preview__group--arrows";
  panGroup.appendChild(createMapControl("↑", "btn btn--nav", () => panMap(0, 64)));
  panGroup.appendChild(createMapControl("↓", "btn btn--nav", () => panMap(0, -64)));
  panGroup.appendChild(createMapControl("←", "btn btn--nav", () => panMap(64, 0)));
  panGroup.appendChild(createMapControl("→", "btn btn--nav", () => panMap(-64, 0)));
  ...
  mapView.pointerUpHandler = (ev) => {
    if (mapView.pointerId != null && ev.pointerId !== mapView.pointerId) return;
    mapView.dragging = false;
    mapView.pointerId = null;
    viewport.classList.remove("is-dragging");
  };
  mapView.resizeHandler = () => fitMapToView();
