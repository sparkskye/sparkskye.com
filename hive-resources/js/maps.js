import { fetchMaps, fetchMapGames, fileViewUrl, trackedDownloadUrl, driveBrowserDownloadUrl } from "./api.js";
import {
  qs,
  debounce,
  setUrlParam,
  ...
  for (const it of items) {
    ...
    name.addEventListener("click", async (ev) => {
      ...
      await downloadViaFetch(dl, filename, {
        button: name,
        fallbackUrl: it.glbId ? driveBrowserDownloadUrl(it.glbId) : "",
      });
    });
  ...
  els.modalDownloadPng.onclick = async (ev) => {
    ...
    if (pngDownload) {
      await downloadViaFetch(pngDownload, pngFilename, {
        button: els.modalDownloadPng,
        fallbackUrl: it.thumbId ? driveBrowserDownloadUrl(it.thumbId) : "",
      });
      return;
    }
    if (it.thumbUrl) {
      await downloadViaFetch(it.thumbUrl, pngFilename, { button: els.modalDownloadPng });
    }
  };
