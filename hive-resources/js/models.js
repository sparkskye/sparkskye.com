import { fetchModels, fileViewUrl, trackedDownloadUrl, fetchDownloadCount } from "./api.js";
import {
  qs,
  debounce,
  setUrlParam,
...
const MAX_CARD_LOAD_CONCURRENCY = 2;
...
const els = {
  ...
  modalDownloads: qs("#modalDownloads"),
};
...
async function openModal(it, opts = {}) {
  ...
  refreshModalDownloadStat(it.modelId);
  ...
  els.modalDownload.onclick = async (ev) => {
    ...
    if (state.activePreviewId === it.modelId) bumpModalDownloadStat();
  };
